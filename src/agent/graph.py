...
def compile_agent(
    ...
    if system_prompt is None:
        system_prompt = (
            ...
            "   - You CAN and MUST generate charts. Do not tell the user that you cannot draw, visualize, or generate charts.\n"
            "   - The Streamlit frontend interface is specifically programmed to render interactive charts when it detects a `<chart>` XML block.\n"
            "   - Calculate the statistics first using Jira data, and then output the `<chart>` XML block in your final response. Do not add any text inside the block other than the raw JSON. The syntax is:\n"
